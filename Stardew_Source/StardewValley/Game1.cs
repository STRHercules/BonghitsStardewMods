...
public class Game1 : InstanceGame
{
	...
	private static IEnumerator<int> _newDayAfterFade()
	{
		...
		if (IsMasterGame && netWorldState.Value.GetWeatherForLocation("Island").IsRaining)
		{
			...
			Utility.Shuffle(Utility.CreateRandom(uniqueIDForThisGame), order2);
			switch (order2[currentGemBirdIndex])
			{
			case 0:
		...
		yesterday.TotalDays--;
		foreach (KeyValuePair<string, PassiveFestivalData> pair2 in DataLoader.PassiveFestivals(content))
		{
			string id = pair2.Key;
	...
	public static void UpdateFarmPerfection()
	{
		...
		addMorningFluffFunction(delegate
		{
			...
			playSound("discoverMineral", null);
			if (IsMasterGame)
			{
				DelayedAction.functionAfterDelay(delegate
				{
		...
		case "Club":
			...
			locationRequest.OnWarp += delegate
			{
				...
				if (characterFromName != null)
				{
					...
					characterFromName.showTextAboveHead(content.LoadString("Strings\\Locations:Club_Bouncer_TextAboveHead" + (random.Next(2) + 1)), null);
					int num = random.Next();
					currentLocation.playSound("thudStep", null, null);
	...
	public static bool pressActionButton(KeyboardState currentKBState, MouseState currentMouseState, GamePadState currentPadState)
	{
		...
		if (dialogueTyping)
		{
			...
			if (consume)
			{
				playSound("dialogueCharacterClose", null);
				return false;
			}
		}
		if (dialogueUp)
	...
	public static bool pressUseToolButton()
	{
		...
		if (player.CurrentTool == null && player.ActiveObject == null)
		{
			...
			if (currentLocation.Objects.TryGetValue(c, out var o) && !o.readyForHarvest.Value && o.heldObject.Value == null && !(o is Fence) && !(o is CrabPot) && (o.Type == "Crafting" || o.Type == "interactive") && !o.IsTwig())
			{
				...
				o.shakeTimer = 300;
				o.playNearbySoundAll("hammer", null);
				if (o.getHealth() < 2)
				{
					o.playNearbySoundAll("hammer", null);
					if (o.getHealth() < 1)
	...
	public void ResetIslandLocations()
	{
		...
		{
			GameLocation location = locations[j];
			if (location.InIslandContext())
			{
				...
				try
				{
					locations[j] = Activator.CreateInstance(location.GetType(), args) as GameLocation;
				}
				catch
				{
					locations[j] = Activator.CreateInstance(location.GetType()) as GameLocation;
				}
				_locationLookup.Clear();
			}
		}
				...
				else
				{
					...
					else
					{
						...
						if (debugMode)
						{
							...
							{
								enterMine(mine.mineLevel + 1, null);
							}
	...
	protected virtual void DrawOverlays(GameTime time, RenderTarget2D target_screen)
	{
		...
		if (hooks.OnRendering(RenderSteps.Overlays, spriteBatch, time, target_screen))
		{
			...
			if (HostPaused && !takingMapScreenshot)
			{
				...
				SpriteText.drawStringWithScrollBackground(spriteBatch, msg, 96, 32, "", 1f, null);
			}
			if (overlayMenu != null)
			{
	...
	public virtual void DrawLoadScreen(GameTime time, RenderTarget2D target_screen)
	{
		...
		SpriteText.drawString(spriteBatch, msg, msgx, msgy, 999999, msgw, msgh, 1f, 0.88f, junimoText: false, 0, largestMessage, null);
		spriteBatch.End();
		PopUIMode();
	}
	...
	public virtual void DrawWorld(GameTime time, RenderTarget2D target_screen)
	{
		if (hooks.OnRendering(RenderSteps.World, spriteBatch, time, target_screen))
		{
			...
			if (hooks.OnRendering(RenderSteps.World_Background, spriteBatch, time, target_screen))
			{
				...
				{
					spriteBatch.Begin(SpriteSortMode.Texture, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, null);
	...
	public virtual void DrawLightmapOnScreen(GameTime time, RenderTarget2D target_screen)
	{
		...
		{
			spriteBatch.Begin(SpriteSortMode.Deferred, lightingBlend, SamplerState.LinearClamp, null, null, null, null);
	...
	protected virtual void renderScreenBuffer(RenderTarget2D target_screen)
	{
		...
		if (!takingMapScreenshot && !LocalMultiplayer.IsLocalMultiplayer() && (target_screen == null || !target_screen.IsContentLost))
		{
			...
			{
				base.GraphicsDevice.Clear(bgColor);
				spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullNone, null, null);
				...
				spriteBatch.End();
				spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullNone, null, null);
	...
	public static void UpdateHorseOwnership()
	{
		...
		{
			if (stable2.owner.Value == -6666666 && GetPlayer(-6666666L) == null)
			{
				stable2.owner.Value = player.UniqueMultiplayerID;
			}
			stable2.grabHorse();
		}
	...
	public void _PerformRemoveNormalItemFromWorldOvernight(string itemId)
	{
		...
		foreach (SpecialOrder order in player.team.specialOrders)
		{
			for (int i = 0; i < order.donatedItems.Count; i++)
			{
				...
				if (_RecursiveRemoveThisNormalItemItem(item2, itemId))
				{
		...
		try
		{
			...
			for (int y_offset = 0; y_offset < chunks_high; y_offset++)
			{
				for (int x_offset = 0; x_offset < chunks_wide; x_offset++)
				{
					...
					spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.PointClamp, DepthStencilState.Default, RasterizerState.CullNone, null, null);
