...
public class NPC : Character, IComparable
{
	...
	private void goblinDoorEndBehavior(Character c, GameLocation l)
	{
		...
		l.playSound("doorClose", null, null);
	}
	...
	private void performRemoveHenchman()
	{
		...
		showTextAboveHead(Game1.content.LoadString("Strings\\Characters:Henchman6"), null);
	...
	public virtual bool tryToReceiveActiveObject(Farmer who, bool probe = false)
	{
		...
		if (base.Name == "Henchman" && Game1.currentLocation.NameOrUniqueName == "WitchSwamp")
		{
			if (activeObj.QualifiedItemId == "(O)308")
			{
				...
				if (!probe)
				{
					who.currentLocation.localSound("coin", null, null);
					who.reduceActiveItemByOne();
		...
		case "(O)897":
			if (!probe)
			{
				if (base.Name == "Pierre" && !Game1.player.hasOrWillReceiveMail("PierreStocklist"))
				{
					...
					who.reduceActiveItemByOne();
					who.completelyStopAnimatingOrDoingAction();
					who.currentLocation.localSound("give_gift", null, null);
				...
				if (dialogue != null)
				{
					if (!probe)
					{
						...
						CurrentDialogue.Peek().onFinish = delegate
						{
							...
							who.reduceActiveItemByOne();
							DelayedAction.playSoundAfterDelay("coin", 200, null, null);
		...
		if (reactingToShorts)
		{
			...
			if (!(text == "Lewis"))
			{
				if (text == "Marnie")
				{
					...
					{
						l.playSound("dustMeep", null, null);
					}));
			...
			else
			{
				...
				Sprite.loop = false;
				shakeTimer = 1000;
				l.playSound("batScreech", null, null);
			}
			return true;
		}
		if (setTemporaryMessages(who))
	...
	public void sayHiTo(Character c)
	{
		...
		{
			showTextAboveHead(getHi(c.displayName), null);
	...
	public virtual void arriveAt(GameLocation l)
	{
		...
		{
			showTextAboveHead(Game1.random.Choose(rawText.Split('/')), null);
		}
	}
	...
	private void startRouteBehavior(string behaviorName)
	{
		...
		case "dick_fish":
			...
			{
				base.currentLocation.playSound("slosh", base.Tile, null);
			}
			break;
		case "clint_hammer":
	...
	public virtual void receiveGift(Object o, Farmer giver, bool updateGiftLimitInfo = true, float friendshipChangeMultiplier = 1f, bool showResponse = true)
	{
		if (CanReceiveGifts())
		{
			...
			giver.currentLocation.localSound("give_gift", null, null);
	...
	public void getHitByPlayer(Farmer who, GameLocation location)
	{
		...
		{
			location.localSound("crafting", null, null);
		}
		else
		{
			location.localSound("hitEnemy", null, null);
		}
	}
	...
	{
		base.currentLocation.playSound("hammer", base.Tile, null);
	}
