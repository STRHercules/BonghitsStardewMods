						...
						if (Math.Abs(chunk.position.X + 32f - (float)playerPixel.X) <= 64f && Math.Abs(chunk.position.Y + 32f - (float)playerPixel.Y) <= 64f)
						{
							...
							if (collect(player.Value, chunk))
							{
								if (Game1.debrisSoundInterval <= 0f)
								{
									...
									{
										location.localSound("coin", null, null);
									}
								}
								chunks.RemoveAt(i);
							}
				...
				else
				{
					...
					if (chunk.position.Y >= (float)chunkFinalYLevel && chunk.hasPassedRestingLineOnce.Value)
					{
						...
						if (stillBouncing)
						{
							...
							else if (debrisType.Value != DebrisType.LETTERS && debrisType.Value != DebrisType.NUMBERS && debrisType.Value != DebrisType.SPRITECHUNKS && (debrisType.Value != 0 || chunkType.Value == 8) && shouldControlThis(location))
							{
								location.playSound("shiny4", null, null);
							}
						}
						if (isSinking.Value)
						{
