...
public class GameLocation : INetObject<NetFields>, IEquatable<GameLocation>, IAnimalLocation, IHaveModData
{
	...
	public virtual void MakeMapModifications(bool force = false)
	{
		...
		case 9:
			...
			case 'B':
				...
				{
					ApplyMapOverride("Backwoods_GraveSite", (Microsoft.Xna.Framework.Rectangle?)null, (Microsoft.Xna.Framework.Rectangle?)null);
				}
				if (Game1.MasterPlayer.mailReceived.Contains("communityUpgradeShortcuts") && !_appliedMapOverrides.Contains("Backwoods_Staircase"))
				{
					ApplyMapOverride("Backwoods_Staircase", (Microsoft.Xna.Framework.Rectangle?)null, (Microsoft.Xna.Framework.Rectangle?)null);
	...
	public void addResourceClumpAndRemoveUnderlyingTerrain(int resourceClumpIndex, int width, int height, Vector2 tile)
	{
		...
		resourceClumps.Add(new ResourceClump(resourceClumpIndex, width, height, tile, null));
	}
	...
	public virtual bool CanWakeUpHere(Farmer who, Point? tile = null)
	{
		...
		{
			return TryGetMapPropertyAs("AllowWakeUpWithoutBed", out allowWakeUpWithoutBed, required: false) && allowWakeUpWithoutBed;
		}
		return true;
	}
	...
	public virtual bool isCollidingPosition(Microsoft.Xna.Framework.Rectangle position, xTile.Dimensions.Rectangle viewport, bool isFarmer, int damagesFarmer, bool glider, Character character, bool pathfinding, bool projectile = false, bool ignoreCharacterRequirement = false, bool skipCollisionEffects = false)
	{
		...
		if (!glider)
		{
			...
			if (_TestCornersTiles(nextTopRight, nextTopLeft, nextBottomRight, nextBottomLeft, nextTopMid, nextBottomMid, currentTopRight, currentTopLeft, currentBottomRight, currentBottomLeft, currentTopMid, currentBottomMid, nextLargerThanTile, (Vector2 corner) => (terrainFeatures.TryGetValue(corner, out var value3) && value3 != null && value3.getBoundingBox().Intersects(position) && !value3.isPassable(character)) ? true : false))
			{
				return true;
			}
		}
		...
		if (!isFarmer || !Game1.player.isRafting)
		{
			if ((!(character is FarmAnimal animal3) || !animal3.IsActuallySwimming()) && _TestCornersTiles(nextTopRight, nextTopLeft, nextBottomRight, nextBottomLeft, nextTopMid, nextBottomMid, currentTopRight, currentTopLeft, currentBottomRight, currentBottomLeft, currentTopMid, currentBottomMid, nextLargerThanTile, delegate(Vector2 tile)
			{
			...
			else
			{
				...
				case 4:
					...
					case 'D':
					{
						...
						for (int k = 1; k < action.Length && (!(action[k] == "Sebastian") || !IsGreenRainingHere() || Game1.year != 1); k++)
						{
					...
					case 'P':
						if (actionType == "PoolEntrance")
						{
							if (!Game1.player.swimming.Value)
							{
								...
								playSound("pullItemFromWater", null, null);
				...
				case 8:
					if (actionType == "asdlfkjg")
					{
						...
						if (Game1.timeOfDay >= 1920 && Game1.timeOfDay < 2020 && farmers.Count == 1 && Game1.stats.DaysPlayed > 3 && !Game1.isRaining && Game1.random.NextDouble() < 0.025)
						{
							...
							playSound("shadowDie", null, null);
							DelayedAction.playSoundAfterDelay("grassyStep", 500, this, null);
							DelayedAction.playSoundAfterDelay("grassyStep", 1000, this, null);
							DelayedAction.playSoundAfterDelay("grassyStep", 1500, this, null);
				...
				case 14:
					...
					if (Game1.player.ActiveObject?.QualifiedItemId == "(O)74" && !Game1.player.mailReceived.Contains("galaxySword"))
					{
						...
						playSound("crit", null, null);
						Game1.screenGlowOnce(new Color(30, 0, 150), hold: true, 0.01f, 0.999f);
						DelayedAction.playSoundAfterDelay("stardrop", 1500, null, null);
					...
					{
						localSound("SpringBirds", null, null);
					}
					break;
				case 10:
	...
	public virtual void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (fishSplashAnimation != null)
		{
			...
			if (frenzy && Game1.random.NextDouble() < 0.005)
			{
				...
				TemporarySprites.Add(new TemporaryAnimatedSprite(fishData.GetTextureName(), fishData.GetSourceRect(0, null), position, flip, 0f, Color.White)
				{
					scale = 4f,
		...
		{
			TerrainFeature feature2 = _activeTerrainFeatures[i2];
			if (feature2.tickUpdate(time))
			{
				terrainFeatures.Remove(feature2.Tile);
			}
		}
	...
	public virtual void DayUpdate(int dayOfMonth)
	{
		...
		for (int i2 = debris.Count - 1; i2 >= 0; i2--)
		{
			Debris d2 = debris[i2];
		...
		objects.Lock();
		foreach (KeyValuePair<Vector2, Object> pair4 in objects.Pairs)
		{
		...
		terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> pair) => pair.Value is HoeDirt hoeDirt && (hoeDirt.crop == null || hoeDirt.crop.forageCrop.Value) && (!objects.TryGetValue(pair.Key, out var value) || value == null || !value.IsSpawnedObject || !value.isForage()) && Game1.random.NextBool(GetDirtDecayChance(pair.Key)));
		lightLevel.Value = 0f;
	...
	public void RespawnStumpsFromMapProperty()
	{
		...
		for (int i = 0; i < stumpData.Length; i += 3)
		{
			...
			if (!foundStump)
			{
				resourceClumps.Add(new ResourceClump(600, 2, 2, tile, null));
	...
	public void setUpLocationSpecificFlair()
	{
			...
			case 'L':
				if (value == "LeahHouse")
				{
					...
					if (l != null && l.currentLocation == this && !l.isDivorcedFrom(Game1.player))
					{
						...
						l.showTextAboveHead(Game1.content.LoadString(toSay10, Game1.player.Name), null);
					}
				}
				break;
			}
			...
			case 'a':
				...
				if (Game1.random.NextDouble() < 0.25)
				{
					...
					if (p8 != null && p8.TilePoint.Y == 18 && p8.currentLocation == this)
					{
						...
						p8.showTextAboveHead(Game1.content.LoadString("Strings\\SpeechBubbles:Saloon_Gus_" + toSay12), null);
					}
				}
			...
			case 'S':
				...
				if (Game1.random.NextBool() && Game1.player.currentLocation != null && Game1.player.currentLocation.isOutdoors.Value)
				{
					...
					if (p != null && p.TilePoint.Y == 18)
					{
						...
						p.showTextAboveHead(Game1.content.LoadString(toSay3, Game1.player.Name), null);
					}
				}
			...
			case 'E':
				if (value == "ElliottHouse")
				{
					...
					if (e != null && e.currentLocation == this && !e.isDivorcedFrom(Game1.player))
					{
						...
						e.showTextAboveHead(Game1.content.LoadString(toSay2, Game1.player.Name), null);
					}
				}
				break;
			}
		...
		case 8:
			...
			case 'H':
			{
				...
				if (p5 != null && p5.currentLocation == this && !p5.isDivorcedFrom(Game1.player))
				{
					...
					{
						p5.showTextAboveHead(Game1.content.LoadString(toSay8), null);
					}
				}
				break;
			}
			case 'J':
				...
				if (Game1.random.NextBool())
				{
					...
					if (p6 != null && p6.currentLocation == this)
					{
						...
						p6.showTextAboveHead(Game1.content.LoadString(toSay9), null);
					}
				}
				break;
			case 'S':
				...
				if (Game1.random.NextBool() && Game1.player.TilePoint.Y > 10)
				{
					...
					if (p4 != null && p4.TilePoint.Y == 17 && p4.currentLocation == this)
					{
						...
						p4.showTextAboveHead(string.Format(dialogue, Game1.player.Name), null);
					}
				}
			...
			case 'A':
				...
				if (Game1.random.NextBool())
				{
					...
					if (p3 != null && p3.TilePoint.Y == 14)
					{
						...
						p3.showTextAboveHead(Game1.content.LoadString(toSay6, Game1.player.Name, Game1.player.farmName), null);
					}
				}
			...
			case 'S':
				...
				if (Game1.random.NextBool())
				{
					...
					if (p2 != null && p2.currentLocation == this)
					{
						...
						p2.showTextAboveHead(Game1.content.LoadString(toSay4), null);
					}
				}
				break;
			case 'M':
				if (value == "ManorHouse")
				{
					...
					if (le != null && le.currentLocation == this)
					{
						...
						le.showTextAboveHead(Game1.content.LoadString("Strings\\SpeechBubbles:ManorHouse_Lewis_" + toSay5), null);
					}
				}
				break;
			case 'G':
		...
		case 14:
			...
			if (Game1.random.NextBool())
			{
				...
				if (p7 != null)
				{
					...
					p7.showTextAboveHead(Game1.content.LoadString(toSay11), null);
				}
			}
			break;
		case 16:
		{
			...
			if (g != null && g.currentLocation == this)
			{
				...
				g.showTextAboveHead(Game1.content.LoadString("Strings\\SpeechBubbles:ArchaeologyHouse_Gunther_" + toSay), null);
			}
			break;
		}
		case 11:
	...
	public virtual bool CanPlaceThisFurnitureHere(Furniture furniture)
	{
		...
		{
			if (!TryGetMapPropertyAs("AllowBeds", out bool allowBedsHere, required: false))
			{
	...
	public void playTerrainSound(Vector2 tileLocation, Character who = null, bool showTerrainDisturbAnimation = true)
	{
		...
		{
			localSound("jingleBell", null, null);
		}
		if (currentStep.Length > 0)
		{
			localSound(currentStep, null, null);
		}
	}
		...
		if (NameOrUniqueName == "SkullCave" && (tileIndex == 344 || tileIndex == 349))
		{
			...
			if (!Game1.player.team.toggleSkullShrineOvernight.Value)
			{
				...
				else
				{
					...
					playSound(Game1.player.team.skullShrineActivated.Value ? "skeletonStep" : "serpentDie", null, null);
				}
			}
			else if (Game1.player.team.toggleSkullShrineOvernight.Value && Game1.player.team.skullShrineActivated.Value)
			{
				...
				playSound("skeletonStep", null, null);
			}
			return true;
		}
			...
			if (obj.Type != null || isErrorItem)
			{
				...
				else if (objects.TryGetValue(tilePos, out obj) && (obj.isSpawnedObject.Value || isErrorItem))
				{
					...
					if (who.couldInventoryAcceptThisItem(obj))
					{
						if (who.IsLocalPlayer)
						{
							localSound("pickUpItem", null, null);
							DelayedAction.playSoundAfterDelay("coin", 300, null, null);
						}
						...
						if (!isFarmBuildingInterior())
						{
							...
							if (obj.ItemId.Equals("789") && Name.Equals("LewisBasement"))
							{
								...
								playSound("cursed_mannequin", null, null);
								characters.Add(b);
							}
						}
						else
	...
	public virtual bool LowPriorityLeftClick(int x, int y, Farmer who)
	{
		...
		for (int i = furniture.Count - 1; i >= 0; i--)
		{
			...
			if (CanFreePlaceFurniture() || furnitureItem.IsCloseEnoughToFarmer(who, null, null))
			{
		...
		for (int i2 = furniture.Count - 1; i2 >= 0; i2--)
		{
			...
			if ((CanFreePlaceFurniture() || furnitureItem2.IsCloseEnoughToFarmer(who, null, null)) && furnitureItem2.isPassable() && furnitureItem2.boundingBox.Value.Contains(x, y) && furnitureItem2.canBeRemoved(who))
			{
	...
	public void openDoor(Location tileLocation, bool playSound)
	{
		try
		{
			...
			if (playSound)
			{
				...
				if (tileIndex == 120)
				{
					this.playSound("doorOpen", pos, null);
				}
				else
				{
					this.playSound("doorCreak", pos, null);
				}
			}
		}
		catch (Exception)
	...
	public void doStarpoint(string which)
	{
		...
		else if (Game1.player.ActiveObject != null && Game1.player.ActiveObject.QualifiedItemId == "(O)307")
		{
			...
			localSound("discoverMineral", null, null);
			Game1.flashAlpha = 1f;
		}
	}
	...
	public virtual void ShowQiCat()
	{
		...
		{
			Game1.player.team.RequestSetMail(PlayerActionTarget.Host, "GotPerfectionStatue", MailType.Received, add: true, null);
	...
	public virtual bool CheckGarbage(string id, Vector2 tile, Farmer who, bool playAnimations = true, bool reactNpcs = true, Action<string> logError = null)
	{
		...
		if (playAnimations)
		{
			...
			if (doubleMega)
			{
				playSound("explosion", null, null);
			}
			else if (mega)
			{
				playSound("crit", null, null);
			}
			playSound("trashcan", null, null);
		...
		if (who.IsLocalPlayer)
		{
			...
			case "SpecialWaterDroppable":
				if (!(this is MineShaft) || (this as MineShaft).mineLevel == 100)
				{
					...
					{
						localSound("throwDownITem", null, null);
						who.reduceActiveItemByOne();
						...
						tempSprite.reachedStopCoordinate = delegate
						{
							...
							localSound("dropItemInWater", null, null);
							DelayedAction.functionAfterDelay(delegate
							{
								localSound("terraria_boneSerpent", null, null);
					...
					if (who?.ActiveObject != null && !who.ActiveObject.questItem.Value && who.ActiveObject.QualifiedItemId != "(O)FarAwayStone" && who.ActiveObject.Edibility <= 0 && !who.ActiveObject.Name.Contains("Totem"))
					{
						...
						if (itemData != null)
						{
							...
							TemporaryAnimatedSprite tempSprite2 = new TemporaryAnimatedSprite(itemData.GetTextureName(), itemData.GetSourceRect(0, null), 9999f, 1, 1, who.position.Value + new Vector2(0f, -128f), flicker: false, flipped: false)
							{
								motion = new Vector2(4f, -4f),
								acceleration = new Vector2(0f, 0.3f),
								yStopCoordinate = (int)who.position.Y,
			...
			case "Warp_Sunroom_Door":
				...
				{
					playSound("doorClose", new Vector2(tileLocation.X, tileLocation.Y), null);
			...
			case "IceCreamStand":
			{
				...
				Utility.TryOpenShopMenu("IceCreamStand", this, npcArea, null);
				break;
			}
			case "WizardShrine":
			...
			case "HospitalShop":
			{
				...
				Utility.TryOpenShopMenu("Hospital", this, ownerSearchArea, null);
				break;
			}
			case "BuyBackpack":
			{
			...
			case "TunnelSafe":
				if (who.ActiveObject?.QualifiedItemId == "(O)787" && !who.hasOrWillReceiveMail("TH_Tunnel"))
				{
					who.reduceActiveItemByOne();
					Game1.player.CanMove = false;
					playSound("openBox", null, null);
					DelayedAction.playSoundAfterDelay("doorCreakReverse", 500, null, null);
			...
			case "SkullDoor":
				if (who.hasSkullKey || Utility.IsPassiveFestivalDay("DesertFestival"))
				{
					if (!who.hasUnlockedSkullDoor && !Utility.IsPassiveFestivalDay("DesertFestival"))
					{
						...
						DelayedAction.playSoundAfterDelay("openBox", 500, null, null);
						DelayedAction.playSoundAfterDelay("openBox", 700, null, null);
					...
					else
					{
						...
						playSound("doorClose", null, null);
						DelayedAction.playSoundAfterDelay("stairsdown", 500, this, null);
			...
			case "playSound":
			{
				...
				localSound(audioName, null, null);
				break;
			}
			case "Letter":
			{
			...
			case "NextMineLevel":
			{
				...
				playSound("stairsdown", null, null);
			...
			case "EnterSewer":
				...
				{
					playSound("stairsdown", new Vector2(tileLocation.X, tileLocation.Y), null);
			...
			case "BuildingSilo":
			{
				...
				if (activeObj?.QualifiedItemId == "(O)178")
				{
					...
					if (stored > 0)
					{
						...
						DelayedAction.playSoundAfterDelay("grassyStep", 100, null, null);
	...
	public void lockedDoorWarp(Point tile, string locationName, int openTime, int closeTime, string npcName, int minFriendship)
	{
		...
		if (canOpenDoor)
		{
			...
			playSound("doorClose", Game1.player.Tile, null);
			Game1.warpFarmer(locationName, tile.X, tile.Y, flip: false);
		}
	...
	public virtual bool HandleBuyAction(string which)
	{
		...
		{
			Utility.TryOpenShopMenu("Sandy", this, null, null);
			return true;
		}
		return false;
	}
	...
	public virtual bool saloon(Location tileLocation)
	{
		...
		if (Utility.TryOpenShopMenu("Saloon", this, shopOwnerArea, null))
		{
	...
	public virtual bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
	{
		...
		case "Fizz_Yes":
			if (Game1.player.Money >= 500000)
			{
				...
				DelayedAction.playSoundAfterDelay("qi_shop_purchase", 500, null, null);
				getCharacterFromName("Fizz")?.showTextAboveHead(Game1.content.LoadString("Strings\\1_6_Strings:Fizz_Sweet"), null);
		...
		case "professionForget_farming":
		{
			...
			DelayedAction.playSoundAfterDelay("dog_bark", 300, null, null);
			DelayedAction.playSoundAfterDelay("dog_bark", 900, null, null);
			break;
		}
		...
		case "evilShrineCenter_Yes":
			if (Game1.player.Money >= 30000)
			{
				...
				playSound("fireball", null, null);
				DelayedAction.playSoundAfterDelay("debuffHit", 500, this, null);
				int count = 0;
				Game1.player.faceDirection(2);
		...
		case "evilShrineRightActivate_Yes":
			if (Game1.player.Items.ReduceId("(O)203", 1) > 0)
			{
				...
				playSound("fireball", null, null);
				DelayedAction.playSoundAfterDelay("batScreech", 500, this, null);
				for (int i = 0; i < 20; i++)
				{
		...
		case "BuyQiCoins_Yes":
			if (Game1.player.Money >= 1000)
			{
				...
				localSound("Pickup_Coin15", null, null);
		...
		case "SleepTent_Yes":
			...
			DelayedAction.playSoundAfterDelay("sandyStep", 500, null, null);
			startSleep();
			break;
		case "Mine_Yes":
	...
	public void playShopPhoneNumberSounds(string whichShop)
	{
		...
		DelayedAction.playSoundAfterDelay("telephone_ringingInEar", 3150, null, null);
	}
	...
	private void communityUpgradeOffer()
	{
		if (!Game1.MasterPlayer.mailReceived.Contains("pamHouseUpgrade"))
		{
			...
			Game1.player.team.RequestSetMail(PlayerActionTarget.Host, "pamHouseUpgradeAsked", MailType.Received, add: true, null);
		}
		else if (!Game1.MasterPlayer.mailReceived.Contains("communityUpgradeShortcuts"))
		{
	...
	public virtual bool sinkDebris(Debris debris, Vector2 chunkTile, Vector2 chunkPosition)
	{
		...
		{
			localSound("quickSlosh", null, null);
			TemporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 150f, 8, 0, chunkPosition + new Vector2(-8f), flicker: false, Game1.random.NextBool(), 0.001f, 0.02f, Color.White, 1f, 0.003f, 0f, 0f));
			return true;
		}
	...
	public virtual bool dropObject(Object obj, Vector2 dropLocation, xTile.Dimensions.Rectangle viewport, bool initialPlacement, Farmer who = null)
	{
		...
		else if (isWaterTile((int)tileLocation.X, (int)tileLocation.Y))
		{
			...
			playSound("dropItemInWater", null, null);
		}
		else
		{
	...
	public virtual void performTenMinuteUpdate(int timeOfDay)
	{
			...
			if (Game1.IsMasterGame)
			{
				...
				if (fishSplashPoint.Value.Equals(Point.Zero) && r.NextBool() && (!(this is Farm) || Game1.whichFarm == 1))
				{
					for (int tries = 0; tries < 2; tries++)
					{
						...
						if (Game1.player.currentLocation.Equals(this))
						{
							playSound("waterSlosh", null, null);
						}
		...
		if (Game1.dayOfMonth % 7 == 0 && Game1.timeOfDay >= 1200 && Game1.timeOfDay <= 1500 && name.Equals("Saloon") && NetWorldState.checkAnywhereForWorldStateID("saloonSportsRoom"))
		{
			...
			else
			{
				...
				{
					n2.showTextAboveHead(Game1.content.LoadString("Strings\\Characters:Saloon_" + (goodEvent ? "goodEvent" : (badEvent ? "badEvent" : "neutralEvent")) + "_" + Game1.random.Next(5)), null);
		...
		foreach (Debris d in debris)
		{
			...
			if (d.itemId.Value != null)
			{
				...
				for (int j = 0; j < d.Chunks.Count; j++)
				{
					...
					Microsoft.Xna.Framework.Rectangle sourceRect = ((d.debrisType.Value == Debris.DebrisType.RESOURCE) ? itemData.GetSourceRect(chunk4.randomOffset, null) : itemData.GetSourceRect(0, null));
