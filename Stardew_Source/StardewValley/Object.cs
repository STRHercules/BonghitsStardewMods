...
public class Object : Item
{
	...
	public virtual bool performToolAction(Tool t)
	{
		...
		if (base.QualifiedItemId == "(BC)165" && heldObject.Value is Chest chest && !chest.isEmpty())
		{
			...
			{
				playNearbySoundAll("hammer", null);
				shakeTimer = 100;
			}
			return false;
		}
		if (t == null)
		{
			if (location.objects.TryGetValue(tileLocation.Value, out var tileObj) && tileObj.Equals(this))
			{
				...
				{
					Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(4, 10), resource: false, -1, item: false, null);
				}
		...
		if (IsBreakableStone() && t is Pickaxe)
		{
			...
			{
				damage = 0;
				playNearbySoundAll("crafting", null);
			}
		...
		else
		{
			...
			{
				fragility.Value = 2;
				playNearbySoundAll("axchop", null);
			...
			if (name.Contains("SupplyCrate") && t.isHeavyHitter())
			{
				...
				{
					fragility.Value = 2;
					playNearbySoundAll("barrelBreak", null);
				...
				shakeTimer = 200;
				playNearbySoundAll("woodWhack", null);
				return false;
			}
		}
		if (base.QualifiedItemId == "(O)590" || base.QualifiedItemId == "(O)SeedSpot")
		{
			if (t is Hoe)
			{
				...
				playNearbySoundAll("hoeHit", null);
		...
		if (Type == "Crafting" && !(t is MeleeWeapon) && t.isHeavyHitter())
		{
			...
			{
				Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(3, 6), resource: false, -1, item: false, null);
				Game1.createRadialDebris(location, 14, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(3, 6), resource: false, -1, item: false, null);
				DelayedAction.functionAfterDelay(delegate
				{
					Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(2, 5), resource: false, -1, item: false, null);
					Game1.createRadialDebris(location, 14, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(2, 5), resource: false, -1, item: false, null);
				}, 80);
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(12, new Vector2(tileLocation.X * 64f, tileLocation.Y * 64f), Color.White, 8, Game1.random.NextBool(), 50f));
				performRemoveAction();
				location.objects.Remove(tileLocation.Value);
	...
	public virtual void cutWeed(Farmer who)
	{
		...
		case "(O)319":
			...
			sound = "breakingGlass";
			animation = 47;
			playNearbySoundAll("drumkit2", null);
			toDrop = null;
			break;
		case "(O)320":
	...
	public virtual bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (IsSprinkler())
		{
			if (heldObject.Value == null && (dropIn.QualifiedItemId == "(O)915" || dropIn.QualifiedItemId == "(O)913"))
			{
				...
				location.playSound("axe", null, null);
		...
		if (base.QualifiedItemId == "(BC)99" && dropIn.QualifiedItemId == "(O)178")
		{
			...
			location.playSound("Ship", null, null);
	...
	public static Item OutputAnvil(Object machine, Item inputItem, bool probe, MachineItemOutput outputData, Farmer player, out int? overrideMinutesUntilReady)
	{
		...
		if (!probe)
		{
			Game1.currentLocation.playSound("metal_tap", null, null);
			DelayedAction.playSoundAfterDelay("metal_tap", 250, null, null);
			DelayedAction.playSoundAfterDelay("metal_tap", 500, null, null);
		}
	...
	private void totemWarp(Farmer who)
	{
		...
		who.playNearbySoundAll("wand", null);
	...
	public void MonsterMusk(Farmer who)
	{
		...
		location.playSound("croak", null, null);
		who.applyBuff("24");
	}
	...
	private void rainTotem(Farmer who)
	{
		...
		Game1.screenGlow = false;
		location.playSound("thunder", null, null);
		who.canMove = false;
		...
		DelayedAction.playSoundAfterDelay("rainsound", 2000, null, null);
	}
	...
	public virtual bool performUseAction(GameLocation location)
	{
		...
		if (name.Contains("Totem"))
		{
			if (normal_gameplay)
			{
				...
				case "(O)886":
				{
					...
					location.playSound("warrior", null, null);
					Game1.player.faceDirection(2);
					Game1.player.CanMove = false;
					Game1.player.temporarilyInvincible = true;
		...
		if (base.QualifiedItemId == "(O)879")
		{
			...
			location.playSound("steam", null, null);
			return true;
		}
		return false;
	}
	...
	protected virtual bool CheckForActionOnFarmComputer(Farmer who, bool justCheckingForActivity = false)
	{
		...
		shakeTimer = 500;
		Location.localSound("DwarvishSentry", null, null);
		who.freezePause = 500;
		DelayedAction.functionAfterDelay(delegate
		{
	...
	protected virtual bool CheckForActionOnMiniObelisk(Farmer who, bool justCheckingForActivity = false)
	{
		...
		foreach (Vector2 v in array)
		{
			if (!location.IsTileBlockedBy(v, CollisionMask.All, CollisionMask.All))
			{
				...
				location.playSound("wand", null, null);
				Game1.displayFarmer = false;
	...
	protected virtual bool CheckForActionOnSlimeBall(Farmer who, bool justCheckingForActivity = false)
	{
		...
		GameLocation location = Location;
		location.objects.Remove(tileLocation.Value);
		DelayedAction.playSoundAfterDelay("slimedead", 40, null, null);
		DelayedAction.playSoundAfterDelay("slimeHit", 100, null, null);
		location.playSound("slimeHit", null, null);
		...
		case "(BC)29":
		{
			...
			foreach (Vector2 offset in adjacentTilesOffsets)
			{
				...
				if (!location.IsTileOccupiedBy(v) && location.isTilePassable(new Location((int)v.X, (int)v.Y), Game1.viewport) && location.isCharacterAtTile(v) == null)
				{
					...
					location.playSound("shadowpeep", null, null);
					break;
				}
			}
			return;
	...
	public virtual bool minutesElapsed(int minutes)
	{
		...
		if (heldObject.Value != null && base.QualifiedItemId != "(BC)165")
		{
			...
			if (MinutesUntilReady <= 0 && (machineData == null || !machineData.OnlyCompleteOvernight || Game1.newDaySync.hasInstance()))
			{
				...
				{
					environment.playSound("dwop", null, null);
				}
				readyForHarvest.Value = true;
		...
		else
		{
			...
			case "(BC)96":
				...
				if (MinutesUntilReady <= 0)
				{
					...
					Game1.player.team.RequestSetMail(PlayerActionTarget.Host, "Capsule_Broken", MailType.Received, add: true, null);
				}
				break;
			case "(BC)141":
				showNextIndex.Value = !showNextIndex.Value;
	...
	public virtual void drawAsProp(SpriteBatch b)
	{
		...
		else
		{
			...
			b.Draw(itemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData2.GetSourceRect(0, null), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
		}
	}
	...
	public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		else if (!Game1.eventUp || (Game1.CurrentEvent != null && !Game1.CurrentEvent.isTileWalkedOn(x, y)))
		{
			...
			if (IsSprinkler())
			{
				if (heldObject.Value != null)
				{
					...
					spriteBatch.Draw(heldItemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0)) + offset2), heldItemData.GetSourceRect(1, null), Color.White * alpha, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)(isPassable() ? bounds.Top : bounds.Bottom) / 10000f + 1E-05f);
				}
		...
		spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, (float)(y * 64 - 64 - 8) + yOffset)), heldItemData2.GetSourceRect(0, null), Color.White * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, base_sort + 1E-05f);
	...
	public virtual bool placementAction(GameLocation location, int x, int y, Farmer who = null)
	{
		...
		if (!bigCraftable.Value && !(this is Furniture))
		{
			...
			if (IsFloorPathItem())
			{
				...
				{
					location.playSound(floorData.PlacementSound, null, null);
				}
				return true;
			}
			...
			if (IsFenceItem())
			{
				...
				{
					location.playSound(fenceData.PlacementSound, null, null);
				}
				return true;
			}
			switch (base.QualifiedItemId)
			...
			case "(O)286":
			{
				...
				location.playSound("thudStep", null, null);
					...
					case 7:
						...
						case '3':
							if (qualifiedItemId == "(BC)163")
							{
								...
								location.playSound("hammer", null, null);
							}
							break;
						case '5':
						{
						...
						case '6':
							...
							location.playSound((base.QualifiedItemId == "(BC)130") ? "axe" : "hammer", null, null);
							return true;
						}
						break;
					case 6:
		...
		if (isSapling())
		{
			...
			{
				location.playSound("dirtyHit", null, null);
				DelayedAction.playSoundAfterDelay("coin", 100, null, null);
				if (IsTeaSapling())
				{
		...
		location.playSound("woodyStep", null, null);
		return true;
		IL_1e1c:
		if (location.objects.ContainsKey(placementTile) || location is MineShaft || location is VolcanoDungeon)
		{
		...
		location.playSound((base.QualifiedItemId == "(BC)BigChest") ? "axe" : "hammer", null, null);
		return true;
	}
