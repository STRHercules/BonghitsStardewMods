...
public class Farm : GameLocation
{
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		if (Game1.whichFarm == 5)
		{
			...
			{
				resourceClumps.Add(new ResourceClump(600, 2, 2, new Vector2(5f, 32f), null));
			}
			if (objects.Length > 0)
			{
				for (int j = 0; j < 6; j++)
		...
		if (Game1.whichFarm == 2)
		{
			for (int x = 0; x < 20; x++)
			{
				for (int y = 0; y < map.Layers[0].LayerHeight; y++)
				{
					...
					{
						resourceClumps.Add(new ResourceClump(600, 2, 2, new Vector2(x, y), null));
					}
				}
			}
		}
		...
		if (tileLocation.X >= shrine_position.X - 1 && tileLocation.X <= shrine_position.X + 1 && tileLocation.Y == shrine_position.Y)
		{
			...
			if (Game1.year >= 3 && grandpaScore.Value > 0 && grandpaScore.Value < 4)
			{
				if (who.ActiveObject?.QualifiedItemId == "(O)72" && grandpaScore.Value < 4)
				{
					...
					DelayedAction.playSoundAfterDelay("yoba", 800, this, null);
	...
	public void showShipment(Item item, bool playThrowSound = true)
	{
		if (playThrowSound)
		{
			localSound("backpackIN", null, null);
		}
		...
		foreach (bool isColorOverlay in array)
		{
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(isColorOverlay ? 1 : 0, null), initialPosition, flipped: false, 0f, Color.White)
				{
					interval = 9999f,
					scale = 4f,
					alphaFade = 0.045f,
	...
	protected override void resetLocalState()
	{
		...
		if (Game1.MasterPlayer.mailReceived.Contains("Farm_Eternal") && !Game1.player.mailReceived.Contains("Farm_Eternal_Parrots") && !IsRainingHere())
		{
			...
			DelayedAction.playSoundAfterDelay("parrot_squawk", 1000, null, null);
			DelayedAction.playSoundAfterDelay("parrot_squawk", 4000, null, null);
			DelayedAction.playSoundAfterDelay("parrot", 3000, null, null);
			DelayedAction.playSoundAfterDelay("parrot", 5500, null, null);
			DelayedAction.playSoundAfterDelay("parrot_squawk", 7000, null, null);
			for (int k = 0; k < 20; k++)
			{
				DelayedAction.playSoundAfterDelay("batFlap", 5000 + k * 250, null, null);
			}
	...
	private void openShippingBinLid()
	{
		if (shippingBinLid != null)
		{
			...
			{
				localSound("doorCreak", null, null);
			}
	...
	private void closeShippingBinLid()
	{
		if (shippingBinLid != null && shippingBinLid.currentParentTileIndex > 0)
		{
			if (shippingBinLid.pingPongMotion != -1 && Game1.currentLocation == this)
			{
				localSound("doorCreakReverse", null, null);
			}
			shippingBinLid.pingPongMotion = -1;
			shippingBinLid.paused = false;
		}
	...
	private void updateShippingBinLid(GameTime time)
	{
		...
		else if (shippingBinLid.currentParentTileIndex == 0 && shippingBinLid.pingPongMotion == -1)
		{
			...
			{
				localSound("woodyStep", null, null);
			}
			shippingBinLid.paused = true;
		}
	...
	private void doLightningStrike(LightningStrikeEvent lightning)
	{
		if (lightning.smallFlash)
		{
			if (Game1.currentLocation.IsOutdoors && !Game1.newDay && Game1.currentLocation.IsLightningHere())
			{
				...
				DelayedAction.playSoundAfterDelay("thunder_small", Game1.random.Next(500, 1500), null, null);
			}
		}
