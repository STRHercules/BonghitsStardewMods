...
public class Multiplayer
{
	...
	protected virtual void _receivePassoutRequest(Farmer farmer)
	{
		...
		bool? flag = lastSleepLocation?.CanWakeUpHere(farmer, null);
	...
	public virtual void processIncomingMessage(IncomingMessage msg)
	{
		...
		case 4:
		{
			...
			request.OnWarp += delegate
			{
				...
				@event.notifyWhenDone = notify_when_done;
				@event.notifyLocationName = location_for_event_check.NameOrUniqueName;
				@event.notifyLocationIsStructure = (request.IsStructure ? ((byte)1) : ((byte)0));
