...
public class Dialogue
{
	...
	protected virtual void parseDialogueString(string masterString, string translationKey)
	{
		...
		for (int i = 0; i < masterDialogueSplit.Length; i++)
		{
			...
			for (int k = 1; k < rawSplit.Length; k += 2)
			{
				string choice = rawSplit[k];
				string response = rawSplit[k + 1];
				if (response.Contains("*"))
				{
					response = response.Replace("**", "<<<<asterisk>>>>").Replace("*", "#$b#").Replace("<<<<asterisk>>>>", "*");
				}
				playerResponses.Add(new NPCDialogueResponse(null, -1, "quickResponse" + k, Game1.parseText(choice)));
			...
			bool choseOne = false;
			for (int l = 0; l < prerequisiteSplit.Length; l++)
			{
				if (farmer.DialogueQuestionsAnswered.Contains(prerequisiteSplit[l]))
				{
					choseOne = true;
					break;
				}
			...
			dialogues.Add(new DialogueLine("", delegate
			{
				...
				if (!ArgUtility.TryGet(array, 0, out var value, out var error2, allowBlank: false, "string topicId") || !ArgUtility.TryGetOptionalInt(array, 1, out var value2, out error2, 4, "int daysDuration"))
				{
