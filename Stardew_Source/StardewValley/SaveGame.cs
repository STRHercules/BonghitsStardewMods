...
public class SaveGame
{
	...
	public static IEnumerator<int> getLoadEnumerator(string file)
	{
		...
		if (loaded.builders != null)
		{
			...
			{
				SaveablePair<string, BuilderData> pair2 = array[i];
		...
		if (loaded.farmerFriendships != null)
		{
			...
			{
				SaveablePair<FarmerPair, Friendship> pair3 = array2[i];
		...
		if (loaded.limitedNutDrops != null)
		{
			...
			{
				SaveablePair<string, int> pair4 = array3[i];
				if (pair4.Value > 0)
				{
		...
		if (loaded.globalInventories != null)
		{
			...
			{
				SaveablePair<string, Item[]> pair5 = array4[i];
		...
		if (loaded.cellarAssignments != null)
		{
			...
			{
				SaveablePair<int, long> pair6 = array5[i];
		...
		bool num = loaded.lostBooksFound < 0;
		loaded = null;
	...
	{
		Dictionary<string, string> npcNamesByFormerName = GetFormerNpcNames((string newName, CharacterData _) => Game1.getCharacterFromName(newName) == null);
		foreach (KeyValuePair<string, Tuple<NPC, GameLocation>> pair in lostVillagers)
		{
			...
			if (Game1.getCharacterFromName(npc.Name) == null && (!npcNamesByFormerName.TryGetValue(npc.Name, out var newName2) || Game1.getCharacterFromName(newName2) == null) && NPC.TryGetData(newName2 ?? npc.Name, out var _))
			{
				GameLocation home = null;
				string oldName = npc.Name;
				npc.Name = newName2 ?? oldName;
				_ = npc.DefaultMap;
