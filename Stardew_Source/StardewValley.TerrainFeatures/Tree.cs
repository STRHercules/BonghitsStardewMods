...
public class Tree : TerrainFeature
{
			...
			if ((double)Math.Abs(shakeRotation) > Math.PI / 2.0)
			{
				...
				if (data != null)
				{
					...
					if (data.DropWoodOnChop)
					{
						...
						Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, numToDrop, resource: true, -1, item: false, null);
						Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * (double)(12 + extraWoodCalculator(tileLocation))), resource: false, -1, item: false, null);
					}
					if (data.DropWoodOnChop)
					{
			...
			if (growthStage.Value >= 5)
			{
				...
				if (hasSeed.Value && (Game1.IsMultiplayer || Game1.player.ForagingLevel >= 1))
				{
					...
					if (data != null && data.SeedDropItems?.Count > 0)
					{
						...
						{
							Item seed = TryGetDrop(drop, Game1.random, Game1.player, "SeedDropItems", null, null);
							if (seed != null)
							{
				...
				}
				{
					...
					{
						Item item = TryGetDrop(entry, Game1.random, Game1.player, "ShakeItems", null, null);
						if (item != null)
						{
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		else if (growthStage.Value >= 3)
		{
			if (t != null && t.Name.Contains("Ax"))
			{
				...
				location.debris.Add(new Debris(12, Game1.random.Next(t.upgradeLevel.Value * 2, t.upgradeLevel.Value * 4), t.getLastFarmerToUse().GetToolLocation() + new Vector2(16f, 0f), Utility.PointToVector2(t.getLastFarmerToUse().StandingPixel), 0, null));
			}
			else if (explosion <= 0)
			{
				return false;
		...
		else if (growthStage.Value >= 1)
		{
			if (explosion > 0)
			{
				location.playSound("cut", null, null);
				return true;
			}
	...
	public bool fertilize()
	{
		...
		if (growthStage.Value >= 5)
		{
			...
			location.playSound("cancel", null, null);
			return false;
		}
		if (fertilized.Value)
		{
	...
	protected bool TryGetTapperOutput(List<WildTreeTapItemData> tapItems, string previousItemId, Random r, float timeMultiplier, out Object output, out int minutesUntilReady)
	{
		if (tapItems != null)
		{
			...
			foreach (WildTreeTapItemData tapData in tapItems)
			{
				...
				Farmer lastHitBy = Game1.GetPlayer(lastPlayerToHit.Value) ?? Game1.MasterPlayer;
				Item item = TryGetDrop(tapData, r, lastHitBy, "TapItems", (string id) => id.Replace("PREVIOUS_OUTPUT_ID", previousItemId), null);
				if (item != null)
				{
	...
	protected void performBushDestroy(Vector2 tileLocation)
	{
		...
		foreach (WildTreeChopItemData drop in data.ChopItems)
		{
			Item item = TryGetDrop(drop, r, lastHitBy, "ChopItems", null, null);
			if (item != null)
			{
	...
	protected bool performTreeFall(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if (!stump.Value)
		{
			...
			{
				location.playSound("treecrack", null, null);
			}
			stump.Value = true;
			health.Value = 5f;
			falling.Value = true;
			...
			if (data != null)
			{
				...
				else
				{
					...
					if (Game1.IsMultiplayer)
					{
						if (data.DropWoodOnChop)
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * 4.0), resource: true, -1, item: false, null);
						}
						List<WildTreeChopItemData> chopItems = data.ChopItems;
						if (chopItems != null && chopItems.Count > 0)
						{
					...
					else
					{
						if (data.DropWoodOnChop)
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * (double)(5 + extraWoodCalculator(tileLocation))), resource: true, -1, item: false, null);
						}
						...
						if (chopItems2 != null && chopItems2.Count > 0)
						{
							...
							{
								Item item2 = TryGetDrop(drop2, r, lastHitBy, "ChopItems", null, null);
								if (item2 != null)
								{
				...
				location.playSound("treethud", null, null);
			}
			if (!falling.Value)
			{
				return true;
