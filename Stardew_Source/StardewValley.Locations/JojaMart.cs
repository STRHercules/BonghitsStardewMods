...
public class JojaMart : GameLocation
{
	...
	private bool signUpForJoja(int response)
	{
		...
		localSound("smallSelect", null, null);
			...
			else if (!Game1.player.mailReceived.Contains("JojaMember"))
			{
				...
				else
				{
					...
					if (Game1.IsMasterGame)
					{
						...
						{
							Morris.setNewDialogue(greeting2 + "_CommunityCenterComplete");
							Game1.drawDialogue(Morris);
						}
						else
						{
							Morris.setNewDialogue(Dialogue.FromTranslation(Morris, greeting2 + "_MembershipAvailable", 5000));
					...
					{
						Morris.setNewDialogue(greeting2 + "_SecondPlayer");
						Game1.drawDialogue(Morris);
					}
				}
			}
