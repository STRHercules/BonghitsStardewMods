...
public class Town : GameLocation
{
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		if (daysUntilCommunityUpgrade.Value > 0)
		{
			...
			if (daysUntilCommunityUpgrade.Value <= 0)
			{
				...
				{
					Game1.player.team.RequestSetMail(PlayerActionTarget.Host, "pamHouseUpgrade", MailType.Received, add: true, null);
					Game1.player.changeFriendship(1000, Game1.getCharacterFromName("Pam"));
				}
				else
				{
					Game1.player.team.RequestSetMail(PlayerActionTarget.Host, "communityUpgradeShortcuts", MailType.Received, add: true, null);
				}
			}
		}
		...
		if (who.mount == null)
		{
			...
			case 2033:
				if (isShowingDestroyedJoja)
				{
					...
					playSound("stairsdown", Game1.player.Tile, null);
			...
			case 599:
				...
				{
					DelayedAction.playSoundAfterDelay("newArtifact", 250, null, null);
	...
	private void marnie_landed(int extra)
	{
		...
		{
			DelayedAction.playSoundAfterDelay("grassyStep", 100 + i, null, null);
		}
	}
	...
	private void mg_disappear(int extra)
	{
		...
		if (t != null)
		{
			...
			for (int i = 0; i < 3200; i += 200)
			{
				DelayedAction.playSoundAfterDelay("snowyStep", 100 + i, null, null);
			}
		}
	}
