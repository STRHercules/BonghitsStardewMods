...
public class FarmHouse : DecoratableLocation
{
	...
	public static void spouseSleepEndFunction(Character c, GameLocation location)
	{
		...
		if (Game1.random.NextDouble() < 0.1)
		{
			...
			{
				npc.showTextAboveHead(Game1.content.LoadString("Strings\\1_6_Strings:Spouse_Goodnight0", npc.getTermOfSpousalEndearment(Game1.random.NextDouble() < 0.1)), null);
			}
			else
			{
				npc.showTextAboveHead(Game1.content.LoadString("Strings\\1_6_Strings:Spouse_Goodnight1"), null);
			}
		}
	}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		foreach (Vector2 offset in adjacentTilesOffsets)
		{
			...
			if (isCharacterAtTile(v) is Monster monster)
			{
				...
				localSound("swordswipe", null, null);
				spouse.shake(500);
				spouse.showTextAboveHead(Game1.content.LoadString("Strings\\Locations:FarmHouse_SpouseAttacked" + (Game1.random.Next(12) + 1)), null);
	...
	public virtual void AddCellarTiles()
	{
		...
		ApplyMapOverride("FarmHouse_Cellar", "cellar", null, null);
	}
	...
	protected virtual void _ApplyRenovations()
	{
		...
		if (upgradeLevel >= 2)
		{
			...
			{
				ApplyMapOverride("FarmHouse_Bedroom_Open", "bedroom_open", null, null);
			}
			else
			{
				ApplyMapOverride("FarmHouse_Bedroom_Normal", "bedroom_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_SouthernRoom_Add", "southernroom_open", null, null);
			}
			else
			{
				ApplyMapOverride("FarmHouse_SouthernRoom_Remove", "southernroom_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_CornerRoom_Add", "cornerroom_open", null, null);
			...
			{
				ApplyMapOverride("FarmHouse_CornerRoom_Remove", "cornerroom_open", null, null);
				if (displayingSpouseRoom)
				{
			...
			{
				ApplyMapOverride("FarmHouse_DiningRoom_Add", "diningroom_open", null, null);
			}
			else
			{
				ApplyMapOverride("FarmHouse_DiningRoom_Remove", "diningroom_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_Cubby_Add", "cubby_open", null, null);
			}
			else
			{
				ApplyMapOverride("FarmHouse_Cubby_Remove", "cubby_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_FarUpperRoom_Add", "farupperroom_open", null, null);
			}
			else
			{
				ApplyMapOverride("FarmHouse_FarUpperRoom_Remove", "farupperroom_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_ExtendedCornerRoom_Add", "extendedcorner_open", null, null);
			}
			else if (hasOwner && owner.mailReceived.Contains("renovation_corner_open"))
			{
				ApplyMapOverride("FarmHouse_ExtendedCornerRoom_Remove", "extendedcorner_open", null, null);
			}
			...
			{
				ApplyMapOverride("FarmHouse_DiningRoomWall_Add", "diningroomwall_open", null, null);
			}
			else if (hasOwner && owner.mailReceived.Contains("renovation_dining_open"))
			{
				ApplyMapOverride("FarmHouse_DiningRoomWall_Remove", "diningroomwall_open", null, null);
			}
		}
	...
	protected override void resetLocalState()
	{
		...
		if (!Game1.player.activeDialogueEvents.ContainsKey("sebastianFrog2") && Game1.random.NextBool())
		{
			...
			{
				DelayedAction.playSoundAfterDelay("croak", 1000, null, null);
			}
		}
	}
