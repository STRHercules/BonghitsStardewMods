...
public class MineShaft : GameLocation
{
	...
	{
		mineLevel = level;
		name.Value = GetLevelName(level, null);
	...
	public void spawnFlyingMonsterOffScreen()
	{
		...
		case 0:
			if (mineLevel > 10 && isDarkArea())
			{
				...
				playSound("batScreech", null, null);
			}
			break;
		case 10:
			if (GetAdditionalDifficulty() > 0)
		...
		case 121:
			if (mineLevel < 171 || Game1.random.NextBool())
			{
				...
				playSound("serpentDie", null, null);
			}
			else
			{
				characters.Add(BuffMonsterIfNecessary(new Bat(spawnLocation * 64f, mineLevel)
		...
		case 77377:
			...
			playSound("rockGolemHit", null, null);
			break;
		}
	}
				...
				for (int k = 0; k < backLayer2.LayerHeight; k++)
				{
					...
					if (isTileClearForMineObjects(j, k))
					{
						...
						else if (mineRandom.NextDouble() <= 0.005 && !isDarkArea() && !mustKillAllMonstersToAdvance() && (GetAdditionalDifficulty() <= 0 || (getMineArea() == 40 && mineLevel % 40 < 30)))
						{
							...
							resourceClumps.Add(new ResourceClump(whichClump, 2, 2, objectPos4, null));
						}
	...
	{
		DelayedAction.playSoundAfterDelay("fallDown", 800, this, null);
		DelayedAction.playSoundAfterDelay("clubSmash", 1800, null, null);
	...
	private void afterFall()
	{
		...
		Game1.enterMine(mineLevel + lastLevelsDownFallen, null);
	...
	public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		if (who.IsLocalPlayer)
		{
			...
			case 173:
				Game1.enterMine(mineLevel + 1, null);
			...
			case 194:
				...
				Game1.createRadialDebris(this, 382, tileLocation.X, tileLocation.Y, 6, resource: false, -1, item: true, null);
	...
	public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
	{
		...
		SpriteText.drawString(b, (Game1.player.team.calicoEggSkullCavernRating.Value + 1).ToString() ?? "", (int)eggPos.X + 28 - SpriteText.getWidthOfString((Game1.player.team.calicoEggSkullCavernRating.Value + 1).ToString() ?? "") / 2, (int)eggPos.Y + 4, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
	}
	...
	public static MineShaft GetMine(string name)
	{
		...
		if (forceLayout.HasValue)
		{
			name = GetLevelName(mineLevel, null);
		}
		foreach (MineShaft mine in activeMines)
		{
			if (mine.Name == name)
