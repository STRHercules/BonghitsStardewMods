...
public class DesertFestival : Desert
{
	...
	public override bool answerDialogueAction(string question_and_answer, string[] question_params)
	{
		...
		case "Gil_EggRating_Yes":
			...
			DelayedAction.playSoundAfterDelay("coin", 500, null, null);
		...
		if (question_and_answer.StartsWith("Cook"))
		{
			...
			else if (question_and_answer.StartsWith("Cook_ChoseSauce"))
			{
				...
				Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
				{
					...
					DelayedAction.playSoundAfterDelay("hammer", 800, this, null);
					DelayedAction.playSoundAfterDelay("hammer", 1200, this, null);
					DelayedAction.playSoundAfterDelay("hammer", 1600, this, null);
					DelayedAction.playSoundAfterDelay("hammer", 2000, this, null);
					DelayedAction.playSoundAfterDelay("furnace", 2500, this, null);
					for (int i = 0; i < 12; i++)
					{
					...
					DelayedAction.functionAfterDelay(delegate
					{
						...
						Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
						{
							...
							food.customBuff = () => new Buff("DesertFestival", food.Name, food.Name, 600 * Game1.realMilliSecondsPerGameMinute, null, -1, effects, null);
			...
			else if (!(question_and_answer == "Cook_Intro_Yes"))
			{
				if (question_and_answer == "Cook_Intro2_Yes")
				{
					...
					for (int k = 0; k < 5; k++)
					{
						ingredients[k] = new Response(k.ToString() ?? "", Game1.content.LoadString("Strings\\1_6_Strings:Cook_Ingredient" + k));
					}
	...
	public void CactusGuyRevealCactus(int seed)
	{
		...
		DelayedAction.playSoundAfterDelay("thudStep", 500, null, null);
		DelayedAction.playSoundAfterDelay("thudStep", 750, null, null);
		DelayedAction.playSoundAfterDelay(random.ChooseFrom(sounds), 1000, null, null);
		DelayedAction.functionAfterDelay(delegate
		{
	...
	public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		...
		case 796:
		case 797:
			Utility.TryOpenShopMenu("Traveler", this, null, null);
			return true;
		case 792:
		case 793:
			playSound("pig", null, null);
			return true;
		case 1073:
	...
	public static void SetupFestivalDay()
	{
		...
		characters.RemoveAll((NPC character) => !store_data_sheet.ContainsKey(festival_id + "_" + character.Name) || (character.Name == "Leo" && !Game1.MasterPlayer.mailReceived.Contains("leoMoved")) || character.getMasterScheduleRawData().ContainsKey(festival_id + "_" + day_number));
		...
		if (characters.Count > 0)
		{
			...
			SetupMerchantSchedule(character3, 0);
		}
		if (characters.Count > 0)
		{
			NPC character4 = r.ChooseFrom(characters);
			characters.Remove(character4);
			SetupMerchantSchedule(character4, 1);
		}
