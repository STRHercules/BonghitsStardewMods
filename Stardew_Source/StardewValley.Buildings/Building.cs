...
public class Building : INetObject<NetFields>, IHaveModData
{
	...
	public virtual void ToggleAnimalDoor(Farmer who)
	{
		...
		if (sound != null)
		{
			who.currentLocation.playSound(sound, null, null);
		}
	...
	public virtual bool doAction(Vector2 tileLocation, Farmer who)
	{
		...
		else
		{
			...
			if (who.IsLocalPlayer && tileLocation.X == (float)(humanDoor.X + tileX.Value) && tileLocation.Y == (float)(humanDoor.Y + tileY.Value) && interior != null)
			{
				...
				{
					who.currentLocation.playSound("doorClose", tileLocation, null);
	...
	public static void PerformObeliskWarp(string destination, int warp_x, int warp_y, bool force_dismount, Farmer who)
	{
		...
		who.currentLocation.playSound("wand", null, null);
	...
	public virtual void performActionOnConstruction(GameLocation location, Farmer who)
	{
		...
		location.localSound("axchop", buildingCenter, null);
		...
		else
		{
			...
			location.localSound("wand", buildingCenter, null);
		...
		if (data != null)
		{
			foreach (Chest chest in buildingChests)
			{
				...
				if (chestData.DisplayTile.X != -1f && chestData.DisplayTile.Y != -1f && chest.Items.Count > 0 && chest.Items[0] != null)
				{
					...
					b.Draw(itemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(drawX + 32f + 4f, drawY + 32f + yOffset)), itemData.GetSourceRect(0, null), Color.White * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, (sortY + 1f) / 10000f);
				}
			}
			if (data.DrawLayers != null)
			{
