...
public class ParrotUpgradePerch : INetObject<NetFields>
{
				...
				if (flyAway || firstBounce || height < 12f || t < 0.5f)
				{
					if (flapFrame == 0)
					{
						if (_perch.upgradeName.Value != "Hut")
						{
							...
							{
								_perch.locationRef.Value.localSound("parrot_flap", null, null);
							}
							if (!flyAway && !firstBounce)
							{
								if (_perch.upgradeName.Value.Contains("Volcano"))
								{
									...
									{
										_perch.locationRef.Value.localSound("hammer", null, null);
									}
								}
								else if (_perch.upgradeName.Value == "Turtle")
								{
									if (Game1.random.NextDouble() < 0.15000000596046448)
									{
										_perch.locationRef.Value.localSound("hitEnemy", null, null);
									}
								}
								else
								{
									...
									{
										_perch.locationRef.Value.localSound("axe", null, null);
									}
									if (Game1.random.NextDouble() < 0.05000000074505806)
									{
										_perch.locationRef.Value.localSound("dirtyHit", null, null);
									}
									if (Game1.random.NextDouble() < 0.05000000074505806)
									{
										_perch.locationRef.Value.localSound("crafting", null, null);
									}
								}
							}
						}
	...
	public virtual void PerformCompleteAnimation()
	{
		...
		{
			DelayedAction.playSoundAfterDelay("secret1", 800, null, null);
		}
