...
public class Butterfly : Critter
{
	...
	public Butterfly(GameLocation location, Vector2 position, bool islandButterfly = false, bool forceSummerButterfly = false, int baseFrameOverride = -1, bool prismatic = false)
	{
		...
		if (isLit)
		{
			lightId = GenerateLightSourceId(Game1.random.Next());
			Game1.currentLightSources.Add(new LightSource(lightId, 10, position + new Vector2(-30.72f, -93.44f), 0.66f, Color.Black * 0.75f, LightSource.LightContext.None, 0L, location.NameOrUniqueName));
		}
	}
