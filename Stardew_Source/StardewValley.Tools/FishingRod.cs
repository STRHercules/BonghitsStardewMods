...
public class FishingRod : Tool
{
	...
	public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
	{
		...
		if (!isFishing && !castedButBobberStillInAir && !pullingOutOfWater && !isNibbling && !hit && !showingTreasure)
		{
			...
			if (location.canFishHere() && location.isTileFishable(tileX, tileY))
			{
				...
				if (who.IsLocalPlayer)
				{
					if (PlayUseSounds)
					{
						location.playSound("dropItemInWater", bobberTile, null);
					}
		...
		{
			location.playSound("pullItemFromWater", bobberTile, null);
		}
		isFishing = false;
		...
		else if (!hit && who.IsLocalPlayer)
		{
			...
			if (PlayUseSounds)
			{
				who.playNearbySoundLocal("FishHit", null);
			}
		}
		return;
		IL_06ed:
	...
	public void playerCaughtFishEndFunction(bool isBossFish)
	{
		...
		else if (recordSize)
		{
			...
			if (!firstCatch)
			{
				who.playNearbySoundLocal("newRecord", null);
			}
		}
		else
		{
			who.playNearbySoundLocal("fishSlap", null);
		}
		if (firstCatch && who.fishCaught.ContainsKey(whichFish.QualifiedItemId))
		{
			...
			who.playNearbySoundLocal("discoverMineral", null);
		}
	}
	...
	private void doPullFishFromWater(BinaryReader argReader)
	{
		...
		if (PlayUseSounds && who.IsLocalPlayer)
		{
			who.currentLocation.playSound("pullItemFromWater", bobberTile, null);
			who.currentLocation.playSound("dwop", bobberTile, null);
		}
		castedButBobberStillInAir = false;
		pullingOutOfWater = true;
		isFishing = false;
		...
		if (!bobber.Equals(Vector2.Zero) && isFishing)
		{
			...
			if (bobberTimeAccumulator > timePerBobberBob)
			{
				if ((!isNibbling && !isReeling) || Game1.random.NextDouble() < 0.05)
				{
					if (PlayUseSounds)
					{
						who.playNearbySoundLocal("waterSlosh", null);
					}
				...
				{
					who.playNearbySoundLocal("bob", null);
				}
			}
	...
	public override void tickUpdate(GameTime time, Farmer who)
	{
		...
		if (castingChosenCountdown > 0f)
		{
			...
			if (castingChosenCountdown <= 0f && who.CurrentTool != null)
			{
				...
				if (PlayUseSounds && who.IsLocalPlayer)
				{
					who.playNearbySoundAll("cast", null);
	...
	private void doStartCasting()
	{
		...
		if (who.IsLocalPlayer)
		{
			if (PlayUseSounds)
			{
				who.playNearbySoundLocal("button1", null);
			}
		...
		if (castingPower > 0.99f && who.IsLocalPlayer)
		{
			...
			if (PlayUseSounds)
			{
				DelayedAction.playSoundAfterDelay("crit", 200, null, null);
			}
		}
	}
	...
	public void openChestEndFunction(int remainingFish)
	{
		...
		{
			who.playNearbySoundLocal("openChest", null);
