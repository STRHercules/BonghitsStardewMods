...
public class MilkPail : Tool
{
	...
	public override bool beginUsing(GameLocation location, int x, int y, Farmer who)
	{
		...
		if (animal?.currentProduce.Value != null && animal.isAdult() && animal.CanGetProduceWithTool(this) && who.couldInventoryAcceptThisItem(animal.currentProduce.Value, 1))
		{
			...
			if (PlayUseSounds)
			{
				who.playNearbySoundLocal("Milking", null);
			}
		}
		...
		else if (who == Game1.player)
		{
			if (PlayUseSounds)
			{
				DelayedAction.playSoundAfterDelay("fishingRodBend", 300, null, null);
				DelayedAction.playSoundAfterDelay("fishingRodBend", 1200, null, null);
			}
			string toSay = null;
			if (animal != null)
			{
