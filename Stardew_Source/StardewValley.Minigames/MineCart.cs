...
public class MineCart : IMinigame
{
	...
	public class MineDebris : Entity
	{
		...
		protected override void _Update(float time)
		{
			if (timeBeforeDisplay > 0f)
			{
				...
				if (startSound != null)
				{
					Game1.playSound(startSound, null);
				}
			}
			position.X += _dX * time;
			position.Y += _dY * time;
			...
			if (_age >= _lifeTime)
			{
				...
				{
					Game1.playSound(destroySound, null);
				}
				Destroy();
				return;
			}
	...
	public bool tick(GameTime time)
	{
		...
		if (gameState == GameStates.Ingame)
		{
			...
			if (checkpointPositions.Count > 0)
			{
				...
				while (i2 < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[i2] * tileSize))
				{
					...
					{
						if (entity3 is CheckpointIndicator indicator && (int)(indicator.position.X / (float)tileSize) == checkpointPositions[i2])
						{
							indicator.Activate();
							break;
						}
			...
			if (_goalIndicator != null && distanceToTravel != -1 && player.position.X >= _goalIndicator.position.X && distanceToTravel != -1 && player.position.Y <= _goalIndicator.position.Y * (float)tileSize + 4f && !reachedFinish && fadeDelta < 0f)
			{
				...
				foreach (Entity entity2 in _entities)
				{
					if (entity2 is GoalIndicator indicator2)
					{
						indicator2.Activate();
					}
		...
		for (int i3 = sparkShower.Count - 1; i3 >= 0; i3--)
		{
			sparkShower[i3].dy += 0.105f * (delta_time / 0.0166666f);
			sparkShower[i3].x += sparkShower[i3].dx * (delta_time / 0.0166666f);
			sparkShower[i3].y += sparkShower[i3].dy * (delta_time / 0.0166666f);
			sparkShower[i3].c.B = (byte)(0.0 + Math.Max(0.0, Math.Sin(totalTimeMS / (Math.PI * 20.0 / (double)sparkShower[i3].dx)) * 255.0));
			if (reachedFinish)
			{
				sparkShower[i3].c.R = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 50.0) / (Math.PI * 20.0 / (double)sparkShower[i3].dx)) * 255.0));
				sparkShower[i3].c.G = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 100.0) / (Math.PI * 20.0 / (double)sparkShower[i3].dx)) * 255.0));
				if (sparkShower[i3].c.R == 0)
				{
					sparkShower[i3].c.R = byte.MaxValue;
				}
				if (sparkShower[i3].c.G == 0)
				{
					sparkShower[i3].c.G = byte.MaxValue;
				}
			}
			if (sparkShower[i3].y > (float)screenHeight)
			{
				sparkShower.RemoveAt(i3);
			}
		}
		return false;
	}
	...
	public void EndCutscene()
	{
		...
		if (gameMode == 3 && currentTheme == 7)
		{
			...
			Game1.currentMinigame = null;
			DelayedAction.playSoundAfterDelay("discoverMineral", 1000, null, null);
		}
	}
