	...
	public bool tick(GameTime time)
	{
		...
		if (motionPause <= 0)
		{
			...
			if (shopping)
			{
				...
				{
					for (int i5 = storeItems.Count - 1; i5 >= 0; i5--)
					{
						KeyValuePair<Rectangle, int> pair = storeItems.ElementAt(i5);
			...
			else if (deathTimer <= 0f && !waitingForPlayerToMoveDownAMap && !shopping && !shootoutLevel)
			{
				...
				for (int p = 0; p < 4; p++)
				{
					...
					case 0:
					{
						...
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(num * 16 * 3, 0, 48, 48), null))
							{
								CowboyMonster monster5 = new CowboyMonster(spawnQueue[p][0].X, new Point(num * TileSize, 0));
								if (whichRound > 0)
								{
					...
					case 1:
					{
						for (int n = 7; n < 10; n++)
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(720, n * TileSize, 48, 48), null))
							{
								CowboyMonster monster3 = new CowboyMonster(spawnQueue[p][0].X, new Point(15 * TileSize, n * TileSize));
								if (whichRound > 0)
								{
				...
				if (waveTimer <= 0 && monsters.Count == 0 && isSpawnQueueEmpty())
				{
					...
					if (whichRound > 0)
					{
						for (int num2 = 0; num2 < monsterChances.Count; num2++)
						{
							_ = monsterChances[num2];
							monsterChances[num2] *= 1.1f;
						}
					}
			...
			for (int i6 = monsters.Count - 1; i6 >= 0; i6--)
			{
				monsters[i6].move(playerPosition, time);
				if (i6 < monsters.Count && monsters[i6].position.Intersects(playerBoundingBox) && playerInvincibleTimer <= 0)
				{
					if (zombieModeTimer <= 0)
					{
						playerDie();
					...
					if (monsters[i6].type != -2)
					{
						addGuts(monsters[i6].position.Location, monsters[i6].type);
						monsters.RemoveAt(i6);
						Game1.playSound("Cowboy_monsterDie", null);
					}
				}
				if (playingWithAbigail && i6 < monsters.Count && monsters[i6].position.Intersects(player2BoundingBox) && player2invincibletimer <= 0)
				{
					Game1.playSound("Cowboy_monsterDie", null);
