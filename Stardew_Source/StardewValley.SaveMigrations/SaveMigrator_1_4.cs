...
public class SaveMigrator_1_4 : ISaveMigrator
{
	...
	public bool ApplySaveFix(SaveFixes saveFix)
	{
		...
		case SaveFixes.StoredBigCraftablesStackFix:
			...
			{
				if (item is Object object3 && object3.bigCraftable.Value && object3.Stack == 0)
				{
					object3.Stack = 1;
				}
				return true;
				...
				if (!object2.preserve.Value.HasValue)
				{
					...
					for (int j = 0; j < suffixes.Length; j++)
					{
						...
						if (object2.Name.EndsWith(text))
						{
							...
							{
								if (current2.InternalName == text2)
								{
									text3 = current2.ItemId;
									break;
								}
							}
							if (text3 != null)
					...
					for (int k = 0; k < prefixes.Length; k++)
					{
						...
						if (object2.Name.StartsWith(text4))
						{
							...
							string text6 = null;
							foreach (ParsedItemData current3 in ItemRegistry.GetObjectTypeDefinition().GetAllData())
							{
								if (current3.InternalName == text5)
								{
									text6 = current3.ItemId;
									break;
								}
							}
							if (text6 != null)
		...
		case SaveFixes.RevealSecretNoteItemTastes:
		{
			...
			for (int l = 0; l < 21; l++)
			{
				if (notesData.TryGetValue(l, out var note) && Game1.player.secretNotesSeen.Contains(l))
				{
		...
		case SaveFixes.TransferNoteBlockScale:
			Utility.ForEachItem(delegate(Item item)
			{
				if (item is Object @object && (@object.QualifiedItemId == "(O)363" || @object.QualifiedItemId == "(O)464"))
				{
					@object.preservedParentSheetIndex.Value = ((int)@object.scale.X).ToString();
				}
				return true;
			});
			return true;
