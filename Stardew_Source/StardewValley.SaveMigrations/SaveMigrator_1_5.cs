...
public class SaveMigrator_1_5 : ISaveMigrator
{
	...
	public bool ApplySaveFix(SaveFixes saveFix)
	{
		...
		case SaveFixes.BedsToFurniture:
			...
			{
				if (location is FarmHouse { HasOwner: var hasOwner } farmHouse2)
				{
					for (int l = 0; l < farmHouse2.map.Layers[0].LayerWidth; l++)
					{
						for (int m = 0; m < farmHouse2.map.Layers[0].LayerHeight; m++)
						{
							if (farmHouse2.doesTileHaveProperty(l, m, "DefaultBedPosition", "Back") != null)
							{
								if (farmHouse2.upgradeLevel == 0)
								{
									farmHouse2.furniture.Add(new BedFurniture(BedFurniture.DEFAULT_BED_INDEX, new Vector2(l, m)));
								}
								else
								{
									...
									farmHouse2.furniture.Add(new BedFurniture(itemId, new Vector2(l, m)));
								}
							}
						}
					}
