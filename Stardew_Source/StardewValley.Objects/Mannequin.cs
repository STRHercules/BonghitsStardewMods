...
public class Mannequin : Object
{
	...
	private void emitGhost()
	{
		...
		Location.playSound("cursed_mannequin", null, null);
	}
				...
				else
				{
					...
					if (Game1.random.NextDouble() < 0.33)
					{
						for (int i = 0; i < 30; i++)
						{
							...
							{
								Vector2 spot = new Vector2(xPos, j);
								if (Location.isTileLocationOpen(spot) && Location.isTilePlaceable(spot) && !dec_location2.isTileOnWall(xPos, j) && !Location.IsTileOccupiedBy(spot))
								{
									facing.Value = 2;
									renderCache = null;
					...
					for (int k = 0; k < 30; k++)
					{
						...
						for (int l = xStartingPoint; l != xEndingPoint; l += xDirection)
						{
							Vector2 spot2 = new Vector2(l, yPos);
							if (Location.isTileLocationOpen(spot2) && Location.isTilePlaceable(spot2) && !dec_location2.isTileOnWall(l, yPos) && !Location.IsTileOccupiedBy(spot2))
							{
