...
public class BreakableContainer : Object
{
	...
	public override bool performToolAction(Tool t)
	{
		...
		if (t != null && t.isHeavyHitter())
		{
			...
			if (health.Value <= 0)
			{
				...
				{
					playNearbySoundAll(breakSound.Value, null);
				}
			...
			{
				shakeTimer = 300;
				playNearbySoundAll(hitSound.Value, null);
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		spriteBatch.Draw(data.GetTexture(), destination, data.GetSourceRect(showNextIndex.Value ? 1 : 0, null), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 1) / 10000f));
	}
}
