...
public class WoodChipper : Object
{
	...
	public override void addWorkingAnimation()
	{
		...
		if (environment != null && environment.farmers.Any() && Game1.random.NextDouble() < 0.35)
		{
			...
			environment.playSound("woodchipper_occasional", null, null);
			shakeTimer = 1500;
		}
	}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		spriteBatch.Draw(baseTexture, destination, baseDraw.GetSourceRect(readyForHarvest.Value ? 1 : 0, null), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, draw_layer);
		if (shakeTimer > 0)
		{
		...
		if (depositedItem.Value != null && shakeTimer > 0 && _isAnimatingChip)
		{
			...
			spriteBatch.Draw(itemTexture, draw_position, itemDraw.GetSourceRect(0, null), Color.White * alpha, 0f, new Vector2(8f, 8f), 4f * draw_scale, depositedItem.Value.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, draw_layer + 0.00175f);
		}
		...
		if (heldObject.Value != null)
		{
			...
			spriteBatch.Draw(itemTexture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, (float)(y * 64 - 64 - 8) + yOffset)), itemDraw2.GetSourceRect(0, null), Color.White * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, (float)((y + 1) * 64) / 10000f + 1E-05f + tileLocation.X / 10000f);
