...
public class ItemPedestal : Object
{
	...
	public override void draw(SpriteBatch b, int x, int y, float alpha = 1f)
	{
		...
		b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, position), itemData.GetSourceRect(0, null), Color.White, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (position.Y - 2f) / 10000f));
		if (match.Value)
		{
			b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, position), itemData.GetSourceRect(1, null), successColor.Value, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (position.Y - 1f) / 10000f));
		}
		if (heldObject.Value != null)
		{
