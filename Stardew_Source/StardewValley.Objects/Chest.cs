...
public class Chest : Object
{
	...
	public override bool performToolAction(Tool t)
	{
		...
		{
			Location.playSound("woodWhack", null, null);
			for (int i = 0; i < 8; i++)
			{
	...
	public void destroyAndDropContents(Vector2 pointToDropAt)
	{
		...
		{
			location.playSound("throwDownITem", null, null);
		}
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		if (dropInItem != null && dropInItem.QualifiedItemId != base.QualifiedItemId && dropInItem.HasContextTag("swappable_chest") && HasContextTag("swappable_chest") && Location != null)
		{
			if (!probe)
			{
				...
				Location.playSound("axchop", null, null);
			}
			return true;
		}
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		else if (playerChest.Value)
		{
			...
			GetMutex().RequestLock(delegate
			{
				...
				else
				{
					...
					Game1.playSound(fridge.Value ? "doorCreak" : "openChest", null);
					Game1.player.Halt();
					Game1.player.freezePause = 1000;
				}
			});
		...
		else if (!playerChest.Value)
		{
			...
			{
				location.playSound("openChest", null, null);
	...
	public override void updateWhenCurrentLocation(GameTime time)
	{
		...
		if (playerChest.Value)
		{
			...
			else if (((frameCounter.Value == -1 && currentLidFrame > startingLidFrame.Value) || currentLidFrame >= getLastLidFrame()) && Game1.activeClickableMenu == null && GetMutex().IsLockHeld())
			{
				...
				environment.localSound("doorCreakReverse", null, null);
			}
		}
		else
		{
	...
	public virtual void UpdateFarmerNearby(bool animate = true)
	{
		...
		else if (Game1.gameMode != 6)
		{
			if (_farmerNearby)
			{
				location.localSound("doorCreak", null, null);
			}
			else
			{
				location.localSound("doorCreakReverse", null, null);
			}
		}
	}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (playerChest.Value && (base.QualifiedItemId == "(BC)130" || base.QualifiedItemId == "(BC)232" || base.QualifiedItemId.Equals("(BC)BigChest") || base.QualifiedItemId.Equals("(BC)BigStoneChest")))
		{
			if (playerChoiceColor.Value.Equals(Color.Black))
			{
				...
				spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData.GetSourceRect(0, null), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
		...
		if (playerChest.Value)
		{
			...
			spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData3.GetSourceRect(0, null), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
	...
	public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f, bool local = false)
	{
		...
		{
			ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
			spriteBatch.Draw(itemData.GetTexture(), local ? new Vector2(x, y - 64) : Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (y - 1) * 64)), itemData.GetSourceRect(0, null), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, local ? 0.89f : ((float)(y * 64 + 4) / 10000f));
			return;
		}
