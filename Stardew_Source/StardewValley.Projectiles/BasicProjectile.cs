...
public class BasicProjectile : Projectile
{
		...
		: this()
	{
		...
		{
			location?.playSound(firingSound, null, null);
		}
	}
	...
	public override void behaviorOnCollisionWithPlayer(GameLocation location, Farmer player)
	{
		...
		if (debuff.Value != null && player.CanBeDamaged() && Game1.random.Next(11) >= player.Immunity && !player.hasBuff("28") && !player.hasTrinketWithID("BasiliskPaw"))
		{
			...
			location.playSound(debuffSound.Value, null, null);
		}
	...
	protected virtual void explosionAnimation(GameLocation location)
	{
		...
		else
		{
			...
			if (itemId.Value != null)
			{
				...
				Game1.createRadialDebris(location, whichDebris, (int)(position.X + 32f) / 64, (int)(position.Y + 32f) / 64, 6, resource: false, -1, item: false, null);
			}
			else
			{
		...
		{
			location.playSound(collisionSound.Value, null, null);
		}
		if (explode.Value)
		{
