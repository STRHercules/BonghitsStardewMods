	...
	public virtual void PerformDefaultHorseFootstep(string step_type)
	{
		...
		if (!(step_type == "Stone"))
		{
			if (step_type == "Wood")
			{
				...
				{
					rider.currentLocation.localSound("woodyStep", base.Tile, null);
				}
			...
			else
			{
				if (rider.ShouldHandleAnimationSound())
				{
					rider.currentLocation.localSound("thudStep", base.Tile, null);
				}
				if (rider == Game1.player)
				{
		...
		else
		{
			if (rider.ShouldHandleAnimationSound())
			{
				rider.currentLocation.localSound("stoneStep", base.Tile, null);
			}
			if (rider == Game1.player)
			{
				Rumble.rumble(0.1f, 50f);
