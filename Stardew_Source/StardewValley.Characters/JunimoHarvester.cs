				...
				else if (oldTimer >= 1000 && harvestTimer < 1000)
				{
					...
					if (base.currentLocation != null && hut != null && !hut.noHarvest.Value && isHarvestable())
					{
						...
						else if (bush.readyForHarvest())
						{
							...
							if (Utility.isOnScreen(base.TilePoint, 64, base.currentLocation))
							{
								DelayedAction.playSoundAfterDelay("coin", 260, base.currentLocation, null);
							}
						}
						if (lastItemHarvested != null)
						{
							...
							if (base.currentLocation.farmers.Any())
							{
								...
								if (gotDouble)
								{
									for (int j = 0; j < 2; j++)
									{
										Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(0, null), 1000f, 1, 0, base.Position + new Vector2(0f, -40f), flicker: false, flipped: false, mainDrawLayer, 0.02f, Color.White, 4f, -0.01f, 0f, 0f)
										{
										...
										{
											Rectangle colored_source_rect = ItemRegistry.GetDataOrErrorItem(lastItemHarvested.QualifiedItemId).GetSourceRect(1, null);
								...
								else
								{
									...
									{
										Rectangle colored_source_rect2 = ItemRegistry.GetDataOrErrorItem(lastItemHarvested.QualifiedItemId).GetSourceRect(1, null);
