...
public class Monster : NPC
{
	...
	public int takeDamage(int damage, int xTrajectory, int yTrajectory, bool isBomb, double addedPrecision, string hitSound)
	{
		...
		{
			Health -= actualDamage;
			base.currentLocation.playSound(hitSound, null, null);
	...
	public override void MovePosition(GameTime time, xTile.Dimensions.Rectangle viewport, GameLocation currentLocation)
	{
		...
		if (moveUp)
		{
			...
			else
			{
				...
				else if (base.willDestroyObjectsUnderfoot)
				{
					...
					{
						currentLocation.playSound("stoneCrack", null, null);
						position.Y -= (float)base.speed + addedSpeed;
					}
					else
					{
		...
		if (!(text == "Shadow Guy"))
		{
			if (text == "Ghost")
			{
				currentLocation.playSound("ghost", null, null);
			}
		}
		else if (Game1.random.NextDouble() < 0.3)
		{
			if (Game1.random.NextBool())
			{
				currentLocation.playSound("grunt", null, null);
			}
			else
			{
				currentLocation.playSound("shadowpeep", null, null);
			}
		}
	}
