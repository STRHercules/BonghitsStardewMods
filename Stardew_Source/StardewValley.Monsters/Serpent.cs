...
public class Serpent : Monster
{
	...
	public override int takeDamage(int damage, int xTrajectory, int yTrajectory, bool isBomb, double addedPrecision, Farmer who)
	{
		...
		else
		{
			...
			wasHitCounter = 500;
			base.currentLocation.playSound("serpentHit", null, null);
			if (base.Health <= 0)
			{
				killer.Value = who;
				deathAnimation();
	...
	protected override void localDeathAnimation()
	{
		...
		if (IsRoyalSerpent())
		{
			base.currentLocation.localSound("serpentDie", null, null);
			for (int i = -1; i < segments.Count; i++)
			{
				...
				TemporaryAnimatedSprite current_sprite = new TemporaryAnimatedSprite(Sprite.textureName.Value, source_rect, 800f, 1, 0, segment_position, flicker: false, flipped: false, 0.9f, 0.001f, segment_color, 4f * scale.Value, 0.01f, current_rotation + (float)Math.PI, (float)((double)Game1.random.Next(3, 5) * Math.PI / 64.0))
				{
