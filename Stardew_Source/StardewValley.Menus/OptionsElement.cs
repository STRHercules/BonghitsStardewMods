...
public class OptionsElement : IScreenReadable
{
	...
	public virtual void draw(SpriteBatch b, int slotX, int slotY, IClickableMenu context = null)
	{
		...
		if (whichOption == -1)
		{
			SpriteText.drawString(b, label, slotX + bounds.X + (int)labelOffset.X, slotY + bounds.Y + (int)labelOffset.Y + 56 - SpriteText.getHeightOfString(label), 999, -1, 999, 1f, 0.1f, junimoText: false, -1, "", null);
			return;
		}
