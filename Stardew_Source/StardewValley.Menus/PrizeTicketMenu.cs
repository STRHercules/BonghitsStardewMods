...
public class PrizeTicketMenu : IClickableMenu
{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (mainButton.containsPoint(x, y) && pressedButtonTimer <= 0f && !movingRewardTrack)
		{
			...
			if (Game1.player.Items.CountId("PrizeTicket") > 0)
			{
				gettingReward = true;
				getRewardTimer = 0f;
				DelayedAction.playSoundAfterDelay("discoverMineral", 750, null, null);
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		SpriteText.drawString(b, ticketCount, xPositionOnScreen + 360 - SpriteText.getWidthOfString(ticketCount) / 2, yPositionOnScreen + 276, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
		mainButton.draw(b);
		base.draw(b);
		drawMouse(b);
	}
