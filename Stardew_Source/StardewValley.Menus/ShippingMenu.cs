...
public class ShippingMenu : IClickableMenu
{
		...
		if (outro)
		{
			...
			else if (outroPauseBeforeDateChange > 0)
			{
				...
				if (outroPauseBeforeDateChange <= 0)
				{
					...
					{
						DelayedAction.playSoundAfterDelay(Game1.IsRainingHere() ? "rainsound" : "rooster", 1500, null, null);
					}
		...
		if (introTimer >= 0)
		{
			...
			if (num % 500 < introTimer % 500 && introTimer <= 3000)
			{
				...
				if (categoryThatPoppedUp < 6 && categoryThatPoppedUp > -1)
				{
					...
					{
						Game1.playSound(getCategorySound(categoryThatPoppedUp), null);
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (currentPage == -1)
		{
			...
			if (scroll_draw_y >= 0)
			{
				SpriteText.drawStringWithScrollCenteredAt(b, Utility.getYesterdaysDate(), Game1.uiViewport.Width / 2, scroll_draw_y, "", 1f, null);
			}
			...
			foreach (ClickableTextureComponent c in categories)
			{
				if (introTimer < 2500 - i2 * 500)
				{
					...
					SpriteText.drawString(b, c.hoverText, (int)start.X - itemSlotWidth - categoryLabelsWidth + 8, (int)start.Y + 4, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
					for (int m = 0; m < 6; m++)
					{
		...
		else
		{
			...
			for (int n = currentTab * itemsPerCategoryPage; n < currentTab * itemsPerCategoryPage + itemsPerCategoryPage; n++)
			{
				if (categoryItems[currentPage].Count > n)
				{
					...
					SpriteText.drawString(b, dotsAndName, (int)position.X + 64 + 12, (int)position.Y + 12, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
					SpriteText.drawString(b, totalStr, totalPosX, (int)position.Y + 12, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
					position.Y += 68f;
				}
			}
			backButton.draw(b);
		...
		if (outro)
		{
			...
			SpriteText.drawStringWithScrollCenteredAt(b, newDayPlaque ? Utility.getDateString() : Utility.getYesterdaysDate(), Game1.uiViewport.Width / 2, dayPlaqueY, "", 1f, null);
