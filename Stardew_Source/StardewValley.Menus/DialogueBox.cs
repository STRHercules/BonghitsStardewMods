...
public class DialogueBox : IClickableMenu
{
	...
	public void drawPortrait(SpriteBatch b)
	{
		...
		if (width >= 642)
		{
			...
			SpriteText.drawStringHorizontallyCenteredAt(b, speaker.getName(), xPositionOfPortraitArea + widthOfPortraitArea / 2, portraitBoxY + 296 + 16, 999999, -1, 999999, 1f, 0.88f, junimoText: false, null);
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (isQuestion)
		{
			...
			SpriteText.drawString(b, getCurrentString(), x + 8, y + 12 - (heightForQuestions - height), characterIndexInDialogue, width - 16, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
			if (characterIndexInDialogue >= getCurrentString().Length - 1)
			{
				int responseY = y - (heightForQuestions - height) + SpriteText.getHeightOfString(getCurrentString(), width - 16) + 48;
				for (int i = 0; i < responses.Length; i++)
				{
					...
					SpriteText.drawString(b, responses[i].responseText, x + 8, responseY, 999999, width, 999999, (selectedResponse == i) ? 1f : 0.6f, 0.88f, junimoText: false, -1, "", null);
					responseY += SpriteText.getHeightOfString(responses[i].responseText, width) + 16;
				}
			}
		}
		else
		{
			...
			{
				SpriteText.drawString(b, getCurrentString(), x + 8, y + 8, characterIndexInDialogue, width, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
			}
		}
		if (isPortraitBox() && !isQuestion)
		{
			...
			if (!isQuestion)
			{
				SpriteText.drawString(b, getCurrentString(), x + 8, y + 8, characterIndexInDialogue, width - 460 - 24, 999999, 1f, 0.88f, junimoText: false, -1, "", null);
			}
		}
		...
		{
			SpriteText.drawStringWithScrollBackground(b, hoverText, friendshipJewel.Center.X - SpriteText.getWidthOfString(hoverText) / 2, friendshipJewel.Y - 64, "", 1f, null);
		}
		drawMouse(b);
	}
}
