...
public static class ItemQueryResolver
{
	...
	public static class DefaultResolvers
	{
		...
		public static IEnumerable<ItemQueryResult> LOST_BOOK_OR_ITEM(string key, string arguments, ItemQueryContext context, bool avoidRepeat, HashSet<string> avoidItemIds, Action<string, string> logError)
		{
			...
			return TryResolve(arguments, new ItemQueryContext(context, "query 'LOST_BOOK_OR_ITEM'"), ItemQuerySearchMode.All, null, null);
		}
		...
		public static IEnumerable<ItemQueryResult> MONSTER_SLAYER_REWARDS(string key, string arguments, ItemQueryContext context, bool avoidRepeat, HashSet<string> avoidItemIds, Action<string, string> logError)
		{
			...
			{
				KeyValuePair<string, MonsterSlayerQuestData> pair = array[i];
				string id = pair.Key;
				...
				{
					Item i2 = ItemRegistry.Create(questData.RewardItemId);
			...
			for (int i = 0; i < array.Length; i++)
			{
				KeyValuePair<string, MonsterSlayerQuestData> pair2 = array[i];
				string id = pair2.Key;
				...
				{
					Item i3 = ItemRegistry.Create(questData2.RewardItemId);
			...
			for (int i = 0; i < array.Length; i++)
			{
				KeyValuePair<string, MonsterSlayerQuestData> pair3 = array[i];
				string id = pair3.Key;
				...
				{
					Item i4 = ItemRegistry.Create(questData3.RewardItemId);
			...
			for (int i = 0; i < array.Length; i++)
			{
				KeyValuePair<string, MonsterSlayerQuestData> pair4 = array[i];
				string id = pair4.Key;
				...
				{
					Item i5 = ItemRegistry.Create(questData4.RewardItemId);
					yield return new ItemQueryResult(i5)
					{
		...
		public static IEnumerable<ItemQueryResult> SECRET_NOTE_OR_ITEM(string key, string arguments, ItemQueryContext context, bool avoidRepeat, HashSet<string> avoidItemIds, Action<string, string> logError)
		{
			...
			return TryResolve(arguments, new ItemQueryContext(context, "query 'SECRET_NOTE_OR_ITEM'"), ItemQuerySearchMode.All, null, null);
		}
	...
	public static ItemQueryResult[] TryResolve(string query, ItemQueryContext context, ItemQuerySearchMode filter = ItemQuerySearchMode.All, string perItemCondition = null, int? maxItems = null, bool avoidRepeat = false, HashSet<string> avoidItemIds = null, Action<string, string> logError = null)
	{
		...
		if (ItemResolvers.TryGetValue(queryKey, out var resolver))
		{
			...
			results = results.Where(delegate(ItemQueryResult result)
			{
				...
				if (hashSet2 == null || !hashSet2.Contains(result.Item.QualifiedItemId))
				{
					HashSet<string> hashSet3 = duplicates;
					if (hashSet3 == null || hashSet3.Add(result.Item.QualifiedItemId))
					{
		...
		if (instance != null)
		{
			HashSet<string> hashSet4 = avoidItemIds;
			if (hashSet4 == null || !hashSet4.Contains(instance.QualifiedItemId))
			{
				return new ItemQueryResult[1]
				{
		...
		{
			ItemQueryResult result2 = (context.Random ?? Game1.random).ChooseFrom(results.Where((ItemQueryResult p) => p.Item is Item).ToArray());
